...
--Installing version 0.6 of Application Authorization Database  (Git URL: git@pichub.pifsc.gov:application-development/centralized-tools.git in the auth_app folder)--
	...
	--create the sequences and triggers
	
	begin 
	dsc.dsc_cre_hist_objs_pkg.cre_hist_seq(sys_context( 'userenv', 'current_schema' ), 'AUTH_APP_USER_GROUPS'); 
	dsc.dsc_cre_hist_objs_pkg.cre_hist_trg (sys_context( 'userenv', 'current_schema' ), 'AUTH_APP_USER_GROUPS', 'AUTH_APP_USER_GROUPS', 'Y'); 
	end; 
	/
	begin 
	dsc.dsc_cre_hist_objs_pkg.cre_hist_seq(sys_context( 'userenv', 'current_schema' ), 'AUTH_APP_USERS'); 
	dsc.dsc_cre_hist_objs_pkg.cre_hist_trg (sys_context( 'userenv', 'current_schema' ), 'AUTH_APP_USERS', 'AUTH_APP_USERS', 'Y'); 
	end; 
	/
	begin 
	dsc.dsc_cre_hist_objs_pkg.cre_hist_seq(sys_context( 'userenv', 'current_schema' ), 'AUTH_APP_GROUPS'); 
	dsc.dsc_cre_hist_objs_pkg.cre_hist_trg (sys_context( 'userenv', 'current_schema' ), 'AUTH_APP_GROUPS', 'AUTH_APP_GROUPS', 'Y'); 
	end; 
	/
	...
	--define the upgrade version in the database:
	INSERT INTO DB_UPGRADE_LOGS (UPGRADE_APP_NAME, UPGRADE_VERSION, UPGRADE_DATE, UPGRADE_DESC) VALUES ('Authorization Application Database', '0.3', TO_DATE('17-AUG-17', 'DD-MON-YY'), 'Updated the name of the package function to have an "APX_" prefix since it is an APEX-specific function that will not work outside of the APEX framework, the new function name is APX_CUST_AUTH_USER');
...
--Installing version 0.3 of Data Validation Module  (Git URL: Git URL: git@pichub.pifsc.gov:centralized-data-tools/data-validation-module.git)--
	...
	*/              
	...
	--define missing _HIST table comments:
	COMMENT ON TABLE "DVM_ERRORS_HIST"  IS 'Data Errors (history table)
	
	This history table was implemented using the data history tracking package (svn://badfish.pifsc.gov/Oracle/DSC/trunk/apps/db/dsc/dsc_pkgs) to track changes to the Data Errors (DVM_ERRORS) table';
	
	
	--define missing _HIST table comments:
	COMMENT ON TABLE "DVM_ERROR_TYPES_HIST"  IS 'Data Error Types (history table)
	
	This history table was implemented using the data history tracking package (svn://badfish.pifsc.gov/Oracle/DSC/trunk/apps/db/dsc/dsc_pkgs) to track changes to the Error Types (DVM_ERROR_TYPES) table';
	
	
	comment on table DVM_DATA_STREAMS_HIST is 'Data Streams (history table)
	
	This history table was implemented using the data history tracking package (svn://badfish.pifsc.gov/Oracle/DSC/trunk/apps/db/dsc/dsc_pkgs) to track changes to the Data Streams (DVM_DATA_STREAMS) table';
	
	comment on table DVM_ERR_RES_TYPES_HIST is 'Error Resolution Types (history table)
	
	This history table was implemented using the data history tracking package (svn://badfish.pifsc.gov/Oracle/DSC/trunk/apps/db/dsc/dsc_pkgs) to track changes to the Error Resolution Types (DVM_ERR_RES_TYPES) table';
	
	comment on table DVM_ERR_SEVERITY_HIST is 'Error Severity (history table)
	
	This history table was implemented using the data history tracking package (svn://badfish.pifsc.gov/Oracle/DSC/trunk/apps/db/dsc/dsc_pkgs) to track changes to the Error Severity (DVM_ERR_RES_TYPES) table';
	
	comment on table DVM_QC_OBJECTS_HIST is 'Data QC Objects (history table)
	
	This history table was implemented using the data history tracking package (svn://badfish.pifsc.gov/Oracle/DSC/trunk/apps/db/dsc/dsc_pkgs) to track changes to the Data QC Objects (DVM_QC_OBJECTS) table';
