...
create or replace PACKAGE DVM_PKG IS
	...
	/*
		...
		END;
	*/
		PROCEDURE VALIDATE_PARENT_RECORD_SP (P_DATA_STREAM_CODES IN VARCHAR_ARRAY_NUM, P_PK_ID IN NUMBER);
		...
		*/
		...
		PROCEDURE VALIDATE_PARENT_RECORD_RC_SP (P_DATA_STREAM_CODES IN VARCHAR_ARRAY_NUM, P_PK_ID IN NUMBER, P_SP_RET_CODE OUT PLS_INTEGER, P_EXC_MSG OUT VARCHAR2);


		--procedure to generate a SQL query placeholder string based on the P_INPUT_STRING_ARRAY elements that are supplied.  P_PLACEHOLDER_STRING will contain a comma delimited string with generated placholder values, P_PLACEHOLDER_ARRAY will contain the generated placeholder values in separate array elements.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE GENERATE_PLACEHOLDERS_SP (P_INPUT_STRING_ARRAY IN VARCHAR_ARRAY_NUM, P_PLACEHOLDER_STRING OUT CLOB, P_PLACEHOLDER_ARRAY OUT VARCHAR_ARRAY_NUM);

		--procedure to retrieve the data stream information for the supplied data stream code(s).  It will set the values of package variables: PV_DATA_STREAM_PAR_TABLE, PV_DATA_STREAM_PK_FIELD based on the query results.  This procedure will validate if the data stream code(s) exist, parent table exists in the database, if the parent table has been enabled, and if there are multiple data stream codes specified if the corresponding parent tables are not the same.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE RETRIEVE_DATA_STREAM_INFO_SP;

		--procedure to retrieve a parent record based off of the data stream code(s) (PV_DATA_STREAM_CODES) and PK ID (PV_PK_ID) package variable values.  This procedure will validate if the parent record exists.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE RETRIEVE_PARENT_REC_SP;

		--package procedure that retrieves a parent issue record based on the PV_DATA_STREAM_PAR_TABLE and PV_PK_ID values and stores the record values in the package variable PV_PTA_ISSUE.
		...
		--package procedure to retrieve all of the QC criteria for the specified validation rule sets (identified by P_RULE_SET_ID_ARRAY) and store them in the PV_ALL_CRITERIA package variable.  This procedure uses the DBMS_SQL package to query the database since the number of validation rule set(s) are only known at runtime.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE RETRIEVE_QC_CRITERIA_SP (P_RULE_SET_ID_ARRAY IN NUM_ARRAY);

		--The package procedure creates a new parent issue record (DVM_PTA_ISSUES).
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE DEFINE_PARENT_ISSUE_REC_SP;

		--This procedure updates the parent record (identified by PV_DATA_STREAM_PAR_TABLE and PV_PK_ID) to reference the specified parent issue record (identified by PV_PTA_ISSUE.PTA_ISS_ID).
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE ASSOC_PARENT_ISSUE_REC_SP;

		--The procedure evaluates the QC criteria stored in PV_ALL_CRITERIA package variable for the specified parent record.  The code loops through the different QC views defined in PV_ALL_CRITERIA and processes the specified validation issue criteria for each of them.  The procedure queries for all validation issue records associated with the specified parent record and compares them (using the ISS_TYPE_ID and ISS_DESC fields) to the pending validation issues that were identified by the QC views.  Any of the existing validation issues that do not exist in the list of pending validation issues are deleted from the database (since they no longer exist) and any pending validation issues that do not already exist in the database are inserted.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE EVAL_QC_CRITERIA_SP;
		...
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE PROCESS_QC_CRITERIA_SP (P_BEGIN_POS IN PLS_INTEGER, P_END_POS IN PLS_INTEGER);

		--This procedure populates an issue record variable (ISSUE_REC) with the information based on the issue description and application link templates (specified by QC_CRITERIA_POS) and from the corresponding QC query result set row (specified by CUR_ID).
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE POPULATE_ISSUE_REC_SP (CUR_ID IN OUT NUMBER, QC_CRITERIA_POS IN NUMBER, ISSUE_REC OUT DVM_ISSUES%ROWTYPE);

		--This procedure updates the parent issue record (identified by P_RULE_SET_ID and P_PTA_ISS_ID) to indicate that the parent record was re-evaluated (LAST_EVAL_DATE updated).
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE UPDATE_PTA_RULE_LAST_EVAL_SP (P_RULE_SET_ID IN PLS_INTEGER, P_PTA_ISS_ID IN PLS_INTEGER);
	...
	create or replace PACKAGE BODY DVM_PKG IS
		...
		ISS_TYPE_NAME DVM_CRITERIA_V.ISS_TYPE_NAME%TYPE
		);
...
--		v_data_str_placeholder_array VARCHAR_ARRAY_NUM;
		...
		--Main package procedure that validates a parent record based on the given data stream code(s) defined by P_DATA_STREAM_CODES, and uniquely identified by the specified primary key (P_PK_ID).  This is the main procedure that is called by external programs to validate the specified parent record.  If the procedure is successful it will not commit the changes to the DVM data to allow for flexibility as part of a larger transaction or to allow the user to explicitly commit the changes made by the DVM.  If the procedure is unsuccessful it will rollback the transaction to the state of the beginning of the procedure.
		--Any exceptions encountered during the execution of the procedure will cause the procedure to fail and log/print the error.
		PROCEDURE VALIDATE_PARENT_RECORD_SP (P_DATA_STREAM_CODES IN VARCHAR_ARRAY_NUM, P_PK_ID IN NUMBER) IS
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'Running VALIDATE_PARENT_RECORD_SP()');
...
--					DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'Running RETRIEVE_DATA_STREAM_INFO_SP()');
			...
			RETRIEVE_PARENT_REC_SP;


			--the parent record exists, continue processing:
			--DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'the parent record exists, retrieve the parent issue record()');
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'The parent record for the data stream code(s) and PK was found in the database');
				...
				for i in 1..PV_DATA_STREAM_CODES.COUNT LOOP
					...
					V_TEMP_SQL := 'SELECT COUNT(*) FROM DVM_DS_PTA_RULE_SETS_V WHERE DATA_STREAM_CODE = :DSC AND PTA_ISS_ID = :PEID';

					--execute the query
					EXECUTE IMMEDIATE V_TEMP_SQL into V_NUM_RULE_SETS using PV_DATA_STREAM_CODES(i), PV_PTA_ISSUE.PTA_ISS_ID;

--								DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'There are '||V_NUM_RULE_SETS||' active rule sets for the current data stream(s) and parent issue record (PTA_ISS_ID: '||PV_PTA_ISSUE.PTA_ISS_ID||')');
...
--									DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'this data stream has already been evaluated for the given parent issue record, retrieve the rule_set_id so it can be used for processing');
						--query for the rule_set_id that is associated with the specified parent issue record
						SELECT RULE_SET_ID INTO V_TEMP_RULE_SET_ID FROM DVM_DS_PTA_RULE_SETS_V where DATA_STREAM_CODE = PV_DATA_STREAM_CODES(i) AND PTA_ISS_ID = PV_PTA_ISSUE.PTA_ISS_ID;
...
--									DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'The existing validation rule set to use is: '||V_TEMP_RULE_SET_ID||', update the last evaluation date');

						--update the DVM_PTA_RULE_SETS to update the LAST_EVAL_DATE due to the validation process being executed:
						UPDATE_PTA_RULE_LAST_EVAL_SP (V_TEMP_RULE_SET_ID, PV_PTA_ISSUE.PTA_ISS_ID);

						--The current existing PTA validation rule set was updated successfully
--									DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'The current existing PTA validation rule set was updated successfully (RULE_SET_ID: '||V_TEMP_RULE_SET_ID||'), (PTA_ISS_ID: '||PV_PTA_ISSUE.PTA_ISS_ID||')');
...
--									DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'this data stream (data stream code: '||PV_DATA_STREAM_CODES(i)||') has not been evaluated on the current parent issue record yet, add it to the list of data stream codes that need to be retrieved');
				...
				--DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'check if there are any data stream codes that need to be evaluated using RETRIEVE_ACTIVE_RULE_SET_SP');
				...
				IF (V_TEMP_DATA_STREAM_CODES.COUNT > 0) THEN
					...
					--DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'there is at least one data stream code that needs to be evaluated using RETRIEVE_ACTIVE_RULE_SET_SP');

					--retrieve the active rule sets for the V_TEMP_DATA_STREAM_CODES:
					RETRIEVE_ACTIVE_RULE_SET_SP (V_TEMP_DATA_STREAM_CODES, V_TEMP_RULE_SET_ID_ARRAY);

					--the active rule sets were successfully retrieved:

					--DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'The active rule set(s) were successfully retrieved');
					...
					FOR i in 1..V_TEMP_RULE_SET_ID_ARRAY.COUNT Loop
						...
						--DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'The current rule set ('||V_TEMP_RULE_SET_ID_ARRAY(i)||') was successfully added to the V_RULE_SET_ID_ARRAY');
					END LOOP;
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'The parent issue record was not found in the database, create a new parent issue record');
...
--							DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'Run DEFINE_PARENT_ISSUE_REC_SP(V_RULE_SET_ID_ARRAY)');


				--insert the new parent issue record:
				DEFINE_PARENT_ISSUE_REC_SP;


				--the parent issue record was loaded successfully, proceed with the validation process:
--								DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'The parent issue record was inserted successfully for the data stream code(s) and parent record.  Continue processing: RETRIEVE_ACTIVE_RULE_SET_SP()');
				...
				RETRIEVE_ACTIVE_RULE_SET_SP (PV_DATA_STREAM_CODES, V_RULE_SET_ID_ARRAY);

				--the active rule set(s) were retrieved successfully:

				--DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'The active rule set(s) was retrieved successfully, associate the parent record with the parent issue record');




				--update the parent record to associate it with the new parent issue record (DVM_PTA_ISSUES)
				ASSOC_PARENT_ISSUE_REC_SP;

				--the parent record was updated successfully:
				--DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'The new parent issue record was associated successfully with the validation rule set(s). Continue processing');

			END IF;

--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'The PTA and parent records were processed');

			--retrieve the QC criteria for processing:
			RETRIEVE_QC_CRITERIA_SP (V_RULE_SET_ID_ARRAY);

			--the QC criteria were loaded successfully:

--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'The QC criteria has been loaded successfully, evaluate the QC criteria');
      ...
      EXCEPTION
        ...
        WHEN OTHERS THEN
		  		--catch all other errors:
					...
					DB_LOG_PKG.ADD_LOG_ENTRY ('ERROR', PV_LOG_MSG_HEADER, V_EXCEPTION_MSG);
...
--					DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', PV_LOG_MSG_HEADER, 'The DVM transaction was rolled back');

					--raise a custom application error:
					RAISE_APPLICATION_ERROR (-20237, V_EXCEPTION_MSG);
		...
		--DVM execution procedure for a given parent record (based on P_DATA_STREAM_CODES and P_PK_ID) that provides a return code (P_SP_RET_CODE) with a value that indicates if the DVM executed successfully or not instead of raising an exception.  A P_SP_RET_CODE value of 1 indicates a successful execution and a value of 0 indicates it was not successful.  The P_EXC_MSG parameter will contain the exception message if the P_SP_RET_CODE indicates there was a processing error.  This procedure allows a PL/SQL block to continue even if the DVM has an exception
		PROCEDURE VALIDATE_PARENT_RECORD_RC_SP (P_DATA_STREAM_CODES IN VARCHAR_ARRAY_NUM, P_PK_ID IN NUMBER, P_SP_RET_CODE OUT PLS_INTEGER, P_EXC_MSG OUT VARCHAR2) IS
		...
		BEGIN
			...
			V_TEMP_LOG_SOURCE := 'DVM_PKG.VALIDATE_PARENT_RECORD_RC_SP ((P_DATA_STREAM_CODES(1): '||P_DATA_STREAM_CODES(1)||'), (P_PK_ID: '||P_PK_ID||'))';

			--execute the DVM for the P_PK_ID:
			VALIDATE_PARENT_RECORD_SP (P_DATA_STREAM_CODES, P_PK_ID);
		...
		EXCEPTION
			WHEN OTHERS THEN
				...
				--log the processing error:
				DB_LOG_PKG.ADD_LOG_ENTRY ('ERROR', V_TEMP_LOG_SOURCE, 'The DVM could not be evaluated successfully on the specified Parent Record:'||chr(10)||SQLERRM);


		END VALIDATE_PARENT_RECORD_RC_SP;


		--procedure to generate a SQL query placeholder string based on the P_INPUT_STRING_ARRAY elements that are supplied.  P_PLACEHOLDER_STRING will contain a comma delimited string with generated placholder values, P_PLACEHOLDER_ARRAY will contain the generated placeholder values in separate array elements.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE GENERATE_PLACEHOLDERS_SP (P_INPUT_STRING_ARRAY IN VARCHAR_ARRAY_NUM, P_PLACEHOLDER_STRING OUT CLOB, P_PLACEHOLDER_ARRAY OUT VARCHAR_ARRAY_NUM) IS
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running GENERATE_PLACEHOLDERS_SP()');
      ...
      EXCEPTION
        ...
        WHEN OTHERS THEN
		  		--catch all other errors:
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The placeholder array and comma-delimited list of placeholders was not generated successfully:'||chr(10)|| SQLERRM;

					--log the procedure processing error:
					DB_LOG_PKG.ADD_LOG_ENTRY ('ERROR', V_TEMP_LOG_SOURCE, V_EXCEPTION_MSG);
		...
		--procedure to retrieve the data stream information for the supplied data stream code(s).  It will set the values of package variables: PV_DATA_STREAM_PAR_TABLE, PV_DATA_STREAM_PK_FIELD based on the query results.  This procedure will validate if the data stream code(s) exist, parent table exists in the database, if the parent table has been enabled, and if there are multiple data stream codes specified if the corresponding parent tables are not the same.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE RETRIEVE_DATA_STREAM_INFO_SP IS
			...
			--exception for parent table does not have a single numeric primary key field defined:
			EXC_PAR_TABLE_INV_PK EXCEPTION;
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running RETRIEVE_DATA_STREAM_INFO_SP()');
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'query for the parent table and pk field for the current data stream ('||PV_DATA_STREAM_CODES(i)||')');
				...
				ELSIF (V_PAR_TABLE_ENABLED_YN = 'N') THEN
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The current data stream has a parent table ('||V_TEMP_PAR_TABLE||') that has not been enabled in the DVM (this requires the parent table to have a NUMBER field named PTA_ISS_ID)';
				...
				ELSIF (V_TEMP_PAR_PK_FIELD IS NULL) THEN
					--this is a primary key error, the parent table does not have a single, numeric primary key defined for it.

					--generate the exception message:
					V_EXCEPTION_MSG := 'The current data stream has a parent table ('||V_TEMP_PAR_TABLE||') that does not have a single numeric primary key defined for it, this must be resolved before the DVM can execute successfully';
...
--						DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'this is the first data stream code being checked, store the values in the package variables');
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'the data stream code(s) successfully resolved to one parent table and the data streams were found in the database');
			...
			EXCEPTION
				...
				WHEN NO_DATA_FOUND THEN
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The specified data stream code ('||V_CURRENT_DATA_STREAM||') was not found in the database: '|| SQLERRM;
				...
				WHEN EXC_PAR_TABLE_INV_PK THEN
					--parent table does not have a single numeric primary key defined for it:

					--raise a custom application error:
					RAISE_APPLICATION_ERROR (-20232, V_EXCEPTION_MSG);
				...
				WHEN OTHERS THEN
					...
					--raise a custom application error:
					RAISE_APPLICATION_ERROR (-20205, V_EXCEPTION_MSG);
		...
		--procedure to retrieve a parent record based off of the data stream code(s) (PV_DATA_STREAM_CODES) and PK ID (PV_PK_ID) package variable values.  This procedure will validate if the parent record exists.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE RETRIEVE_PARENT_REC_SP IS
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running RETRIEVE_PARENT_REC_SP()');
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The parent rec query is: '||V_TEMP_SQL);
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The parent record was successfully retrieved');
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running RETRIEVE_PARENT_ISSUE_REC_SP()');
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The query is: '||V_TEMP_SQL);
...
--					DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The parent issue record for the data stream code(s) and parent table PK was not found in the database');
		...
		--package procedure to retrieve all of the QC criteria for the specified validation rule sets (identified by P_RULE_SET_ID_ARRAY) and store them in the PV_ALL_CRITERIA package variable.  This procedure uses the DBMS_SQL package to query the database since the number of validation rule set(s) are only known at runtime.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE RETRIEVE_QC_CRITERIA_SP (P_RULE_SET_ID_ARRAY IN NUM_ARRAY) IS
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running RETRIEVE_QC_CRITERIA_SP()');
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Generate the placeholders for the different validation rule sets');


			--generate the placeholders for the rule_set_id values:
			GENERATE_PLACEHOLDERS_SP (V_TEMP_STRING_ARRAY, V_PLACEHOLDER_STRING, V_PLACEHOLDER_ARRAY);

--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The placeholders were generated successfully, execute the query');
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Query for the QC criteria: '||V_TEMP_SQL);
...
--						DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'the value of V_ALL_CRITERIA_POS is: '||V_ALL_CRITERIA_POS);
				  ...
				  FOR i IN 1 .. COL_CNT LOOP
						...
						ELSIF PV_DESC_TAB(i).col_type IN (1, 96) THEN
							...
							--check the column name to assign it to the corresponding PV_ALL_CRITERIA element record field values
							IF PV_DESC_TAB(i).col_name = 'OBJECT_NAME' THEN  --this is the OBJECT_NAME field, store the character string value of the current record's field in the current PV_ALL_CRITERIA array element for the corresponding variable field
								PV_ALL_CRITERIA(V_ALL_CRITERIA_POS).OBJECT_NAME := NAME_VAR;

--									DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The value of the QC object name is: '||NAME_VAR);

							ELSIF PV_DESC_TAB(i).col_name = 'IND_FIELD_NAME' THEN  --this is the IND_FIELD_NAME field, store the character string value of the current record's field in the current PV_ALL_CRITERIA array element for the corresponding variable field
								PV_ALL_CRITERIA(V_ALL_CRITERIA_POS).IND_FIELD_NAME := NAME_VAR;

							ELSIF PV_DESC_TAB(i).col_name = 'APP_LINK_TEMPLATE' THEN  --this is the APP_LINK_TEMPLATE field, store the character string value of the current record's field in the current PV_ALL_CRITERIA array element for the corresponding variable field
								PV_ALL_CRITERIA(V_ALL_CRITERIA_POS).APP_LINK_TEMPLATE := NAME_VAR;

							ELSIF PV_DESC_TAB(i).col_name = 'ISS_TYPE_NAME' THEN  --this is the ISS_TYPE_NAME field, store the character string value of the current record's field in the current PV_ALL_CRITERIA array element for the corresponding variable field
								PV_ALL_CRITERIA(V_ALL_CRITERIA_POS).ISS_TYPE_NAME := NAME_VAR;
							END IF;
						ELSIF PV_DESC_TAB(i).col_type = 112 THEN
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The RETRIEVE_QC_CRITERIA_SP procedure was executed successfully, the number of validation criteria rules that were loaded is: '||PV_ALL_CRITERIA.COUNT);
			...
			EXCEPTION
				...
				WHEN OTHERS THEN
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The data QC validation rules could not be retrieved succcessfully:'||chr(10)||SQLERRM;
		...
		--The package procedure creates a new parent issue record (DVM_PTA_ISSUES).
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE DEFINE_PARENT_ISSUE_REC_SP IS
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running DEFINE_PARENT_ISSUE_REC_SP()');
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Parent Issue Record inserted successfully (PTA_ISS_ID: '||PV_PTA_ISSUE.PTA_ISS_ID||')');
		...
		--This procedure updates the parent record (identified by PV_DATA_STREAM_PAR_TABLE and PV_PK_ID) to reference the specified parent issue record (identified by PV_PTA_ISSUE.PTA_ISS_ID).
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE ASSOC_PARENT_ISSUE_REC_SP IS
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running ASSOC_PARENT_ISSUE_REC_SP()');
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The query is: '||V_TEMP_SQL);
			...
			EXCEPTION
				...
				WHEN OTHERS THEN
					...
					--construct the exception message:
					V_EXCEPTION_MSG := 'The parent record '||PV_DATA_STREAM_PAR_TABLE||' could not be associated with the new parent issue record:'||chr(10)||SQLERRM;
		...
		--The procedure evaluates the QC criteria stored in PV_ALL_CRITERIA package variable for the specified parent record.  The code loops through the different QC views defined in PV_ALL_CRITERIA and processes the specified validation issue criteria for each of them.  The procedure queries for all validation issue records associated with the specified parent record and compares them (using the ISS_TYPE_ID and ISS_DESC fields) to the pending validation issues that were identified by the QC views.  Any of the existing validation issues that do not exist in the list of pending validation issues are deleted from the database (since they no longer exist) and any pending validation issues that do not already exist in the database are inserted.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE EVAL_QC_CRITERIA_SP IS
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'running EVAL_QC_CRITERIA_SP ()');

			--initialize the tracking variable for the processing loop:
			V_CURRENT_QC_OBJ_ID := NULL;


--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'there are '||PV_ALL_CRITERIA.COUNT||' QC criteria to evaluate');
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'indx ('||indx||'), QC object: '||PV_ALL_CRITERIA(indx).OBJECT_NAME);
...
--					DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'this is not the same QC object as the previous issue type, run PROCESS_QC_CRITERIA_SP on the previous QC criteria object ('||PV_ALL_CRITERIA(indx - 1).OBJECT_NAME||')');

					--process the last QC view object:
					PROCESS_QC_CRITERIA_SP (V_CURR_QC_BEGIN_POS, (indx - 1));
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'execute the PROCESS_QC_CRITERIA_SP() procedure for the last QC object definition: '||PV_ALL_CRITERIA(V_CURR_QC_BEGIN_POS).OBJECT_NAME);

			--there are no processing errors, process the last QC view criteria:
			PROCESS_QC_CRITERIA_SP (V_CURR_QC_BEGIN_POS, PV_ALL_CRITERIA.COUNT);
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'the QC criteria was processed successfully, loop through each of the validation issue records that were defined in the package variable by the POPULATE_ISSUE_REC_SP() procedure');
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'this is not the first time the given record has been validated retrieve the existing validation issues to compare against the pending issue records');
				...
				RETRIEVE_ISSUE_RECS_SP (V_EXISTING_ISSUE_REC_TABLE);

				--the issue records have been retrieved successfully

--					DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'the validation issue records were retrieved successfully');
...
--						DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Looping through the existing issue record ('||V_EXISTING_ISSUE_REC_TABLE(V_EXISTING_ISSUE_REC_COUNTER).ISS_ID||') description: '||V_EXISTING_ISSUE_REC_TABLE(V_EXISTING_ISSUE_REC_COUNTER).ISS_DESC);
...
--							DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Looping through the pending issue record: '||PV_ISSUE_REC_TABLE(V_ISSUE_REC_COUNTER).ISS_DESC);
...
--								DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The current pending validation issue record and existing validation issue record match each other (description and issue type), remove both of them from their respective arrays');
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'all of the comparisons have been made, delete all of the existing issue records that were not matched ('||V_EXISTING_ISSUE_REC_TABLE.COUNT||' total)');
...
--						DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Delete the existing issue record: '||V_EXISTING_ISSUE_REC_TABLE(V_EXISTING_ISSUE_REC_COUNTER).ISS_ID);
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'no processing errors have been encountered so far, insert the DVM validation issue records');
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'insert all of the unmatched pending issue records ('||PV_ISSUE_REC_TABLE.COUNT||' total)');
	...
	--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'insert the issue with issue description: "'||PV_ISSUE_REC_TABLE(V_ISSUE_REC_COUNTER).ISS_DESC);
			...
			EXCEPTION
				...
				WHEN OTHERS THEN
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The data QC validation rules could not be processed for the specified parent record:'||chr(10)||SQLERRM;
		...
		--This procedure executes the QC view criteria stored in PV_ALL_CRITERIA in the elements from P_BEGIN_POS to P_END_POS.  The procedure executes the QC view for the specified parent record (based on PV_PK_ID package variable value) and checks the IND_FIELD_NAME for each the specified validation criteria in the result set to see if they indicate a validation issue (Y) or not (N).  Each identified validation issue calls the POPULATE_ISSUE_REC_SP() procedure to populate the pending validation issue's values and stores them in the PV_ISSUE_REC_TABLE package variable so they can be compared to the existing validation issues and processed accordingly.  This procedure uses the DBMS_SQL package to query the database since the QC View object and corresponding field names is only known at runtime.  This procedure will validate if the QC view exists and if the QC view is invalid.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE PROCESS_QC_CRITERIA_SP (P_BEGIN_POS IN PLS_INTEGER, P_END_POS IN PLS_INTEGER) IS
			...
			--variable to store the current indicator field that is being processed
			V_TEMP_IND_FIELD DVM_ISS_TYPES.IND_FIELD_NAME%TYPE;

			--variable to store the issue type name that is being processed
			V_TEMP_ISS_TYPE_NAME DVM_ISS_TYPES.ISS_TYPE_NAME%TYPE;
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'running PROCESS_QC_CRITERIA_SP (), OBJECT_NAME: '||PV_ALL_CRITERIA(P_BEGIN_POS).OBJECT_NAME);
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'the value of the QC query to be executed is: '||V_TEMP_SQL);
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'fetching column descriptions');
			-- loop through each column and defined the data type of each column dynamically:
			FOR i IN 1 .. COL_CNT LOOP
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'current column name is: '||PV_DESC_TAB(i).col_name);
...
--					DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'current NUM_VAR is: '||NUM_VAR);
...
--					DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'current DATE_VAR is: '||DATE_VAR);

			  ELSIF PV_DESC_TAB(i).col_type IN (1, 96) THEN
					 --define the result set data type as VARCHAR2 for the current column:
					DBMS_SQL.DEFINE_COLUMN(CUR_ID, i, NAME_VAR, 2000);

--					DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'current NAME_VAR is: '||NAME_VAR);

			  END IF;

			END LOOP;

--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'fetching rows');


			-- Fetch rows with DBMS_SQL package to loop through the result set:
			WHILE DBMS_SQL.FETCH_ROWS(CUR_ID) > 0 LOOP

--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'fetching new row from result set');
...
--					DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'current IND_FIELD_NAME is: '||PV_ALL_CRITERIA(j).IND_FIELD_NAME);

					--store the current indicator field so it can be reported if it does not exist:
					V_TEMP_IND_FIELD := PV_ALL_CRITERIA(j).IND_FIELD_NAME;

					--store the current issue type name so it can be reported if it does not exist:
					V_TEMP_ISS_TYPE_NAME := PV_ALL_CRITERIA(j).ISS_TYPE_NAME;
...
--					DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'current IND_FIELD_NAME is: '||PV_ALL_CRITERIA(j).IND_FIELD_NAME||', the corresponding result set value is: '||NAME_VAR);
...
--						DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The IND_FIELD_NAME ('||PV_ALL_CRITERIA(j).IND_FIELD_NAME||') is ''Y'', populate a new pending validation issue record so it can be compared to existing validation issues and inserted if it does not already exist');

					  --populate the issue rec based on the current QC criteria that was evaluated to true:
					  POPULATE_ISSUE_REC_SP (CUR_ID, j, V_TEMP_ISSUE_REC);
...
--						DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The validation issue was added to the array of issues');
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'closing cursor SRC_CUR in PROCESS_QC_CRITERIA_SP()');
			...
			EXCEPTION
				...
				WHEN EXC_MISSING_VIEW THEN
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The data QC view ('||PV_ALL_CRITERIA(P_BEGIN_POS).OBJECT_NAME||') does not exist: ' || SQLERRM;
				...
				WHEN EXC_INVALID_VIEW THEN
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The data QC view ('||PV_ALL_CRITERIA(P_BEGIN_POS).OBJECT_NAME||') is invalid: ' || SQLERRM;
				...
				WHEN NO_DATA_FOUND THEN
					--The indicator field was not in the data QC view result set:

					--generate the exception message:
					V_EXCEPTION_MSG := 'The validation rule ('||V_TEMP_ISS_TYPE_NAME||') indicator field ('||V_TEMP_IND_FIELD||') was not found in the data QC view ('||PV_ALL_CRITERIA(P_BEGIN_POS).OBJECT_NAME||'):'||chr(10)||SQLERRM;

					--log the processing error:
					DB_LOG_PKG.ADD_LOG_ENTRY ('ERROR', V_TEMP_LOG_SOURCE, V_EXCEPTION_MSG);

					--raise a custom application error:
					RAISE_APPLICATION_ERROR (-20234, V_EXCEPTION_MSG);

				WHEN OTHERS THEN
					--catch all other errors:

					--check if the PL/SQL error was due to an unexpected data type (non-character) for an indicator field
					IF (SQLCODE = -06562) THEN

						--generate the exception message:

						V_EXCEPTION_MSG := 'The validation rule ('||V_TEMP_ISS_TYPE_NAME||') indicator field ('||V_TEMP_IND_FIELD||') has an invalid data type in the data QC view ('||PV_ALL_CRITERIA(P_BEGIN_POS).OBJECT_NAME||'), the DVM requires a character data type for indicator fields:'||chr(10)||SQLERRM;

						--log the processing error:
						DB_LOG_PKG.ADD_LOG_ENTRY ('ERROR', V_TEMP_LOG_SOURCE, V_EXCEPTION_MSG);

						--raise a custom application error:
						RAISE_APPLICATION_ERROR (-20235, V_EXCEPTION_MSG);


					ELSIF (SQLCODE = -904) THEN	--check if the PL/SQL error was due to an invalid identifier error (the only identifier provided is the primary key so it means that the primary key field does not exist in the data QC view)

						--generate the exception message:

						V_EXCEPTION_MSG := 'The Data QC View ('||PV_ALL_CRITERIA(P_BEGIN_POS).OBJECT_NAME||') does not contain the primary key field ('||PV_DATA_STREAM_PK_FIELD||') of the parent table ('||PV_DATA_STREAM_PAR_TABLE||'), the DVM requires each Data QC View to include the parent table primary key:'||chr(10)||SQLERRM;
					...
					ELSE
						...
						--generate the exception message:
						V_EXCEPTION_MSG := 'The data QC view ('||PV_ALL_CRITERIA(P_BEGIN_POS).OBJECT_NAME||') could not be processed successfully:'||chr(10)||SQLERRM;
		...
		--This procedure populates an issue record variable (ISSUE_REC) with the information based on the issue description and application link templates (specified by QC_CRITERIA_POS) and from the corresponding QC query result set row (specified by CUR_ID).
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE POPULATE_ISSUE_REC_SP (CUR_ID IN OUT NUMBER, QC_CRITERIA_POS IN NUMBER, ISSUE_REC OUT DVM_ISSUES%ROWTYPE) IS
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running POPULATE_ISSUE_REC_SP (), issue type template is: '||PV_ALL_CRITERIA(QC_CRITERIA_POS).ISS_TYPE_COMMENT_TEMPLATE);
			...
			REPLACE_PLACEHOLDER_VALS_SP (CUR_ID, QC_CRITERIA_POS, PV_ALL_CRITERIA(QC_CRITERIA_POS).ISS_TYPE_COMMENT_TEMPLATE, TEMP_ISS_MESSAGE, FALSE);
			--the placeholders were successfully replaced:

			--DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The issue type template was replaced with runtime values from the query successfully: '||TEMP_ISS_MESSAGE||'. Process the application link template: '||PV_ALL_CRITERIA(QC_CRITERIA_POS).APP_LINK_TEMPLATE);

			--set the TEMP_APP_LINK to the given issue type comment template (APP_LINK_TEMPLATE) based on the current result set values:
			REPLACE_PLACEHOLDER_VALS_SP (CUR_ID, QC_CRITERIA_POS, PV_ALL_CRITERIA(QC_CRITERIA_POS).APP_LINK_TEMPLATE, TEMP_APP_LINK, TRUE);
			--the placeholders were successfully replaced:

			--DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The application link template was replaced with runtime values from the query successfully: '||TEMP_APP_LINK||'.  Populate the pending validation issue record with the replaced values');
			...
			EXCEPTION
				...
				WHEN OTHERS THEN
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The data QC validation issue record could not be successfully populated:'||chr(10)||SQLERRM;
		...
		--This procedure updates the parent issue record (identified by P_RULE_SET_ID and P_PTA_ISS_ID) to indicate that the parent record was re-evaluated (LAST_EVAL_DATE updated).
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE UPDATE_PTA_RULE_LAST_EVAL_SP (P_RULE_SET_ID IN PLS_INTEGER, P_PTA_ISS_ID IN PLS_INTEGER) IS
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running UPDATE_PTA_RULE_LAST_EVAL_SP()');
...
--			DB_LOG_PKG.ADD_LOG_ENTRY('DEBUG', V_TEMP_LOG_SOURCE, 'running QC_MISSING_QUERY_FIELDS_FN()');

			--count the number of occurences of placeholders:
			V_REGEXP_COUNT := REGEXP_COUNT(P_TEMPLATE_VAL, '\[[A-Z0-9\_]{1,}\]');

--			DB_LOG_PKG.ADD_LOG_ENTRY('DEBUG', V_TEMP_LOG_SOURCE, 'There are '||V_REGEXP_COUNT||' placeholder occurrences');
...
--					DB_LOG_PKG.ADD_LOG_ENTRY('DEBUG', V_TEMP_LOG_SOURCE, 'The current placeholder name is: '||V_FIELD_NAME);
...
--					V_FIELD_NAME := SUBSTR(V_FIELD_NAME, 2, LENGTH(V_FIELD_NAME) - 2) ;
		    ...
		    ON ALL_OBJECTS.OBJECT_NAME = ALL_TAB_COLUMNS.TABLE_NAME
				AND ALL_OBJECTS.OWNER = ALL_TAB_COLUMNS.OWNER
		    WHERE
		 		ALL_OBJECTS.OWNER = sys_context( ''userenv'', ''current_schema'' )
				AND ALL_OBJECTS.OBJECT_TYPE = ''VIEW''
		    AND ALL_OBJECTS.OBJECT_NAME = :p01';

--				DB_LOG_PKG.ADD_LOG_ENTRY('DEBUG', V_TEMP_LOG_SOURCE, 'Execute the following query using the P_QC_OBJECT_NAME value ('||P_QC_OBJECT_NAME||') : '||V_TEMP_SQL);
...
--				DB_LOG_PKG.ADD_LOG_ENTRY('DEBUG', V_TEMP_LOG_SOURCE, 'The query was executed successfully, record count: '||V_RESULT_FIELDS.COUNT);
...
--						DB_LOG_PKG.ADD_LOG_ENTRY('DEBUG', V_TEMP_LOG_SOURCE, 'Check if the result field '||V_RESULT_FIELDS(i)||' was found in the list of placeholders');

						--loop through each of the issue templates'
						FOR j in 1.. V_ARRAY_FIELDS.COUNT LOOP


--							DB_LOG_PKG.ADD_LOG_ENTRY('DEBUG', V_TEMP_LOG_SOURCE, 'The current placeholder name is: '||V_ARRAY_FIELDS(j));
	...
	--							DB_LOG_PKG.ADD_LOG_ENTRY('DEBUG', V_TEMP_LOG_SOURCE, 'The current placeholder matches the current field name');
			...
			EXCEPTION
				...
				WHEN EXC_NO_DATA_FOUND THEN
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The data QC view ('||P_QC_OBJECT_NAME||') does not exist: ';
				...
				WHEN OTHERS THEN
					...
					V_EXCEPTION_MSG := 'The data QC view ('||P_QC_OBJECT_NAME||') could not be processed successfully for the '||(CASE WHEN P_APP_LINK_PARAM = 1 THEN 'Application Link' ELSE 'Issue Description' END)||' template ('||(CASE WHEN LENGTH(P_TEMPLATE_VAL) > 300 THEN SUBSTR(P_TEMPLATE_VAL, 1, 300)||'...' ELSE P_TEMPLATE_VAL END) ||') (P_APP_LINK_PARAM: '||P_APP_LINK_PARAM||'):'||chr(10)||SQLERRM;
					...
					RAISE_APPLICATION_ERROR ((CASE WHEN P_APP_LINK_PARAM = 1 THEN -20231 ELSE -20233 END), V_EXCEPTION_MSG);
		...
		--This procedure replaces all template placeholders in P_TEMPLATE_VALUE with the corresponding values in the result set row specified by CUR_ID for the QC criteria identified by QC_CRITERIA_POS and returns the replaced value as P_REPLACED_VALUE.  If the value of the parameter P_APP_LINK_PARAM is true then this is an application link (APP_ID and APP_SESSION placeholders are ignored) and if P_APP_LINK_PARAM is false it is not an application link.  This procedure uses the DBMS_SQL package to query the database since the query fields are only known at runtime.  This procedure will validate if there are template placeholders that are not fields in the corresponding QC view and if a QC view result field has an unexpected data type.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE REPLACE_PLACEHOLDER_VALS_SP (CUR_ID IN OUT NUMBER, QC_CRITERIA_POS IN NUMBER, P_TEMPLATE_VALUE IN VARCHAR2, P_REPLACED_VALUE OUT VARCHAR2, P_APP_LINK_PARAM IN BOOLEAN) IS
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running REPLACE_PLACEHOLDER_VALS_SP()');
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'the template value is not NULL, find the placeholders');
				...
				IF (V_MISSING_FIELDS IS NOT NULL) then
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The current Validation Rule''s '||(CASE WHEN P_APP_LINK_PARAM THEN 'application link' ELSE 'issue description' END)||' template contains one or more placeholders ('||V_MISSING_FIELDS||') that were not found in the data QC view ('||PV_ALL_CRITERIA(QC_CRITERIA_POS).OBJECT_NAME||')';
...
--						DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'there was at least one placeholder found');
...
--								DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'this is not an app link (P_APP_LINK_PARAM = FALSE) or this is an app link (P_APP_LINK_PARAM = TRUE) that is not one of the special APEX placeholders (APP_ID or APP_SESSION) so replace the placeholder with the correponding query field value');

								--replace the placeholder value with the corresponding field value:
--								DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'replace the placeholder value with the corresponding field value');
								...
								ELSE
									...
									--generate the exception message:
									V_EXCEPTION_MSG := 'The '||(CASE WHEN P_APP_LINK_PARAM THEN 'application link' ELSE 'issue description' END)||' template placeholder field ('||V_FIELD_NAME||') for the result set row was an unexpected data type in the associated data QC view: '||PV_DESC_TAB(PV_ASSOC_FIELD_LIST(V_FIELD_NAME)).col_type;
...
--								DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The placeholder field was successfully retrieved');
...
--								DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The value of the replaced value is: '||P_REPLACED_VALUE);
        ...
        WHEN OTHERS THEN
		  		--catch all other errors:
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The '||(CASE WHEN P_APP_LINK_PARAM THEN 'application link' ELSE 'issue description' END)||' template placeholders were not replaced with the corresponding data QC view result set values successfully:'||chr(10)|| SQLERRM;
		...
		--This procedure returns the RULE_SET_ID primary key values for each active validation rule set (DVM_RULE_SETS table) into the P_RULE_SET_ID_ARRAY parameter that corresponds to each data stream code (specified by P_DATA_STREAM_CODES) in the database.  For each data stream it will check if there is an active validation rule set, if not a new validation rule set will be created and associated with the active validation criteria and the primary key (RULE_SET_ID) value will be added to the P_RULE_SET_ID_ARRAY parameter.  If there is an active validation rule set the procedure will check to see if the active rule set is the same as the active set of validation criteria, if so it will add the active validation rule set primary key value to the V_TEMP_RULE_SET_ID_ARRAY parameter; if not the procedure will deactivate the active validation rule set and insert a new validation rule set and associate it with the active validation criteria and the primary key (RULE_SET_ID) value will be added to the P_RULE_SET_ID_ARRAY parameter.  The procedure will associate the parent issue record with the validation rule sets by inserting a record (DVM_PTA_RULE_SETS table) for each identified validation rule sets.  This procedure will validate if there a multiple active validation rules for a specified data stream and if there are no active validation criteria.
		--all error conditions will raise an application exception and will be logged in the database
		procedure RETRIEVE_ACTIVE_RULE_SET_SP (P_DATA_STREAM_CODES IN VARCHAR_ARRAY_NUM, P_RULE_SET_ID_ARRAY OUT NUM_ARRAY) IS
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running RETRIEVE_ACTIVE_RULE_SET_SP()');
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'query for the active validation criteria for the current data stream code ('||P_DATA_STREAM_CODES(i)||')');
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'the number of active validation criteria for the current data stream is: '||TO_CHAR(V_NUM_ACTIVE_CRITERIA));
...
--					DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'There is at least one active validation criteria. V_NUM_ACTIVE_CRITERIA: '||V_NUM_ACTIVE_CRITERIA);
...
--					DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The value of V_NUM_ACTIVE_RULES is: '||V_NUM_ACTIVE_RULES);
...
--						DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'there are no active rules for the current data stream code ('||P_DATA_STREAM_CODES(i)||'), create the new rule set and populate it with the current active rules');

						--insert the new validation rule set:
						DEFINE_RULE_SET_SP (P_DATA_STREAM_CODES(i), P_RULE_SET_ID);

						--the new validation rule set was defined successfully

--							DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'the new rule set was successfully inserted for the data stream code ('||P_DATA_STREAM_CODES(i)||'), rule_set_id: '||P_RULE_SET_ID);
					...
					ELSIF (V_NUM_ACTIVE_RULES > 1) THEN
						...
						--generate the exception message:
						V_EXCEPTION_MSG := 'The data stream code ('||P_DATA_STREAM_CODES(i)||') has more than one active validation rule set associated with it';
...
--						DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'there is only one active rule set, check if it''s the same as the currently active set');
...
--							DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'the values match, use the active rule set for the new PTA issue record');
...
--							DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'the active rule set ID for the data stream ('||P_DATA_STREAM_CODES(i)||') is: '||V_ACT_RULE_SET_ID);
...
--							DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'the current active validation rules do not match the active rule set');
...
--							DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The active rule set was disabled, insert the new active rule set');
...
--								DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'the new rule set was successfully inserted');
				...
				ELSE
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The data stream code ('||P_DATA_STREAM_CODES(i)||') has no associated active validation rules';
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'insert the new DVM_PTA_RULE_SETS record(s) that have not been processed for the specified parent issue record');

			--loop through the rule set IDs and insert the PTA rule set record for the specified parent record and current data stream
			FOR i in 1..P_RULE_SET_ID_ARRAY.COUNT LOOP
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'insert the DVM_PTA_RULE_SETS record for the current rule set');
        ...
        --catch all PL/SQL database exceptions:
				...
				WHEN OTHERS THEN
		  		--catch all other errors:
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The active rule set(s) were not successfully retrieved for the specified data stream code(s): ('||COMMA_DELIM_LIST_FN(P_DATA_STREAM_CODES)||'):'||chr(10)||SQLERRM;
		...
		--This procedure that defines a new validation rule set (DVM_RULE_SETS table) and associates all active issue types (DVM_ISS_TYP_ASSOC table) for the specified data stream (identified by P_DATA_STREAM_CODE) with the new rule set.  The P_RULE_SET_ID parameter will contain the RULE_SET_ID primary key for the new validation rule set record.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE DEFINE_RULE_SET_SP (P_DATA_STREAM_CODE IN VARCHAR2, P_RULE_SET_ID OUT PLS_INTEGER) IS
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running DEFINE_RULE_SET_SP()');
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The Rule set insert query was successful: '||P_RULE_SET_ID);
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The select DVM criteria query is: '||V_TEMP_SQL);
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Insert the issue type ID is: '||V_ISS_TYPE_IDS(i));
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The issue type ID association record was inserted successfully');
        ...
        WHEN OTHERS THEN
		  		--catch all other errors:
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The new validation rule set was not successfully inserted for the specified data stream code ('||P_DATA_STREAM_CODE||'):'||chr(10)||SQLERRM;
		...
		--this procedure queries for all issue records that are related to validation rule set data stream code(s) (specified by PV_DATA_STREAM_CODES package variable) so they can be compared to the pending validation issues that were just identified by the DVM.  The validation issue records are stored in the P_ISSUE_RECS parameter.  This procedure uses the DBMS_SQL package to query the database since the number of data stream code(s) are only known at runtime.
		--all error conditions will raise an application exception and will be logged in the database
		PROCEDURE RETRIEVE_ISSUE_RECS_SP (P_ISSUE_RECS OUT DVM_ISSUES_TABLE) IS
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running RETRIEVE_ISSUE_RECS_SP()');


			--generate the placeholders for the rule_set_id values:
			GENERATE_PLACEHOLDERS_SP (PV_DATA_STREAM_CODES, V_PLACEHOLDER_STRING, V_PLACEHOLDER_ARRAY);


			--query for all QC Validation Issues for the specified DATA_STREAM_CODE values (PV_DATA_STREAM_CODES)
			V_TEMP_SQL := 'SELECT
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'run the query: '||V_TEMP_SQL);
...
--				DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The current col name is: ' ||PV_DESC_TAB(i).col_name || ' col type is: '|| PV_DESC_TAB(i).col_type);
...
--								DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'The current issue_type_id is: '||to_char(NUM_VAR));
...
--		DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'closing CUR_ID cursor in RETRIEVE_ISSUE_RECS_SP()');
        ...
        WHEN OTHERS THEN
		  		--catch all other errors:
					...
					--generate the exception message:
					V_EXCEPTION_MSG := 'The existing data QC validation issues could not be retrieved successfully:'||chr(10)||SQLERRM;
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running INIT_PKG_SP()');
...
--			DB_LOG_PKG.ADD_LOG_ENTRY ('DEBUG', V_TEMP_LOG_SOURCE, 'Running COMMA_DELIM_LIST_FN ()');
      ...
      EXCEPTION
        ...
        WHEN OTHERS THEN
		  		--catch all other errors:
					...
					--check if this is not a -20218 error from the COMMA_DELIM_LIST_FN function call:
					IF (SQLCODE = -20218) THEN
						--this is a -20218 error from the COMMA_DELIM_LIST_FN function call

						--raise the original exception to the calling code block:
						RAISE;

					ELSE
						--this is not a -20218 error from the COMMA_DELIM_LIST_FN function call:
...
INSERT INTO DB_UPGRADE_LOGS (UPGRADE_APP_NAME, UPGRADE_VERSION, UPGRADE_DATE, UPGRADE_DESC) VALUES ('Data Validation Module', '1.5', TO_DATE('04-OCT-23', 'DD-MON-YY'), 'Defined a new aggregate view to provide standard error/warning information for a given DVM parent record');
