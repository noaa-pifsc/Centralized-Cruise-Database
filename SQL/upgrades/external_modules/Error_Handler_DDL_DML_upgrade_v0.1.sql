...
ALTER TABLE APX_ERR_CONSTR_MSG
ADD CONSTRAINT APX_ERR_CONSTR_MSG_U1 UNIQUE 
(
  CONSTRAINT_NAME 
)
ENABLE;

COMMENT ON TABLE APX_ERR_CONSTR_MSG IS 'APEX Custom Database Constraint Error Messages
...
COMMENT ON COLUMN APX_ERR_CONSTR_MSG.CONSTR_MSG_ID IS 'Primary Key for the APX_ERR_CONSTR_MSG table';


COMMENT ON COLUMN APX_ERR_CONSTR_MSG.CONSTRAINT_NAME IS 'The database constraint name the given custom error message is defined for.';

COMMENT ON COLUMN APX_ERR_CONSTR_MSG.ERR_MESSAGE IS 'The user-defined error message that will be shown to the application users when the given CONSTRAINT_NAME is violated';

CREATE SEQUENCE APX_ERR_CONSTR_MSG_SEQ INCREMENT BY 1 START WITH 1;

create or replace TRIGGER APX_ERR_CONSTR_MSG_AUTO_BRI 
before insert on APX_ERR_CONSTR_MSG
for each row
begin
...
create or replace function PIFSC_APX_ERR_HANDLER_FN (
...
begin
    --The PIFSC_APX_ERR_HANDLER_FN is an Oracle function used to suppress sensitive structural error information from database interaction within APEX applications.  It uses a lookup table that allows custom error messages to be defined for each foreign key or unique key constraint name in the data dictionary that is defined in the APX_ERR_CONSTR_MSG table.  There is a flexible data dictionary query (Custom Error Handler DDL.sql) used to populate this table using generic error messages based on the defined APEX workspace UI default definitions or underlying object names.  Other types of common constraint errors are genericized to remove the constraint names and other detailed error messages.  APEX application error and Oracle error messages have the error codes removed.  This function is enabled on all test and production application instances (hard coded to be enabled when the host name is not midd.pic.gov).
    --recommended implementation of function is to deploy by "Edit Application Definition" -> Error Handling Function: PIFSC_APX_ERR_HANDLER_FN which will enable it on all database interaction to suppress detailed structural information in error messages
    ...
    --check if the host name is not midd.pic.gov, if this is a non-development application server then alter all error messages displayed to users:
    IF OWA_UTIL.get_cgi_env ('HTTP_HOST') NOT LIKE 'midd.pic.gov%' THEN
    ...
    return l_result;
end PIFSC_APX_ERR_HANDLER_FN;
/


--define the upgrade version in the database:
INSERT INTO DB_UPGRADE_LOGS (UPGRADE_APP_NAME, UPGRADE_VERSION, UPGRADE_DATE, UPGRADE_DESC) VALUES ('Error Handler', '0.1', TO_DATE('07-JUL-16', 'DD-MON-YY'), 'Initial version of the error handler database that contains a table that stores custom error messages for named constraints and a Oracle function that implements the custom error handling');
