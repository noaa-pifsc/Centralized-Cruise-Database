...
--drop old function that will be replaced by a package function:
drop function PIFSC_APX_ERR_HANDLER_FN;


--rename and modify the table

ALTER TABLE APX_ERR_CONSTR_MSG 
RENAME TO CUST_ERR_CONSTR_MSG;

ALTER TABLE CUST_ERR_CONSTR_MSG RENAME COLUMN CONSTRAINT_NAME TO CONSTR_NAME;

ALTER TABLE CUST_ERR_CONSTR_MSG RENAME COLUMN ERR_MESSAGE TO CONSTR_ERR_MESSAGE;


ALTER INDEX APX_ERR_CONSTR_MSG_PK 
RENAME TO CUST_ERR_CONSTR_MSG_PK;

ALTER INDEX APX_ERR_CONSTR_MSG_U1 
RENAME TO CUST_ERR_CONSTR_MSG_U1;

ALTER TABLE CUST_ERR_CONSTR_MSG 
RENAME CONSTRAINT APX_ERR_CONSTR_MSG_PK TO CUST_ERR_CONSTR_MSG_PK;

ALTER TABLE CUST_ERR_CONSTR_MSG 
RENAME CONSTRAINT APX_ERR_CONSTR_MSG_U1 TO CUST_ERR_CONSTR_MSG_U1;


--drop trigger
drop trigger APX_ERR_CONSTR_MSG_AUTO_BRI;

--drop sequence
drop sequence APX_ERR_CONSTR_MSG_SEQ;


--rename sequence for the new object naming convention:
...
begin
    ...
    select nvl(max(constr_msg_id), 0) into max_id from cust_err_constr_msg;
    
    execute immediate 'CREATE SEQUENCE CUST_ERR_CONSTR_MSG_SEQ INCREMENT BY 1 START WITH '||to_char(max_id + 1);
...
create or replace TRIGGER CUST_ERR_CONSTR_MSG_AUTO_BRI 
before insert on CUST_ERR_CONSTR_MSG
for each row
begin
  select CUST_ERR_CONSTR_MSG_SEQ.nextval into :new.CONSTR_MSG_ID from dual;
end;
/
...
INSERT INTO DB_UPGRADE_LOGS (UPGRADE_APP_NAME, UPGRADE_VERSION, UPGRADE_DATE, UPGRADE_DESC) VALUES ('Error Handler', '0.2', TO_DATE('16-AUG-17', 'DD-MON-YY'), 'Renamed the error constraint message table and created a package for the APEX error handling function to allow additional custom error handling functions/procedures to be added to this module.');
