...
is
	...
	v_dev_hostname CC_CONFIG_OPTIONS.OPTION_VALUE%TYPE;
begin
	...
	--retrieve the development server hostname from the configuration options table:
	SELECT OPTION_VALUE INTO v_dev_hostname	FROM CC_CONFIG_OPTIONS WHERE OPTION_NAME = 'DEV_HOSTNAME';			
    ...
    IF OWA_UTIL.get_cgi_env ('HTTP_HOST') NOT LIKE v_dev_hostname||'%' THEN
...
INSERT INTO DB_UPGRADE_LOGS (UPGRADE_APP_NAME, UPGRADE_VERSION, UPGRADE_DATE, UPGRADE_DESC) VALUES ('Error Handler', '0.5', TO_DATE('31-MAR-23', 'DD-MON-YY'), 'Updated the CUST_ERR_PKG package to use the CC_CONFIG table''s DEV_HOSTNAME option''s value');
