...
create or replace PACKAGE DVM_PKG IS
	...
	*/
	...
	--        DBMS_OUTPUT.PUT_LINE('Running POPULATE_ERROR_REC('||QC_criteria_pos||')');
			...
			--set the temp_error_message to the given error type comment template:
			temp_app_link := ALL_CRITERIA(QC_criteria_pos).APP_LINK_TEMPLATE;
	...
	--        DBMS_OUTPUT.PUT_LINE('The value of temp_error_message before any replacements is: '||temp_error_message);
			...
			WHILE temp_field_name IS NOT NULL LOOP
				...
				IF (desctab(assoc_field_list(temp_field_name)).col_type IN (1, 96)) THEN
				  ...
				  --update the page link by replacing the current column name placeholder to the corresponding runtime value:
				  temp_app_link := REPLACE(temp_app_link, '['||temp_field_name||']', namevar);
	        ...
	        DBMS_OUTPUT.PUT_LINE ('replace ['||temp_field_name||'] with '||TO_CHAR(numvar));
				  ...
				  --update the page link by replacing the current column name placeholder to the corresponding runtime value:
				  temp_app_link := REPLACE(temp_app_link, '['||temp_field_name||']', TO_CHAR(numvar));
	        ...
	        DBMS_OUTPUT.PUT_LINE ('replace ['||temp_field_name||'] with '||TO_CHAR(datevar));
				  ...
				  --update the page link by replacing the current column name placeholder to the corresponding runtime value:
				  temp_app_link := REPLACE(temp_app_link, '['||temp_field_name||']', TO_CHAR(datevar));
...
INSERT INTO DB_UPGRADE_LOGS (UPGRADE_APP_NAME, UPGRADE_VERSION, UPGRADE_DATE, UPGRADE_DESC) VALUES ('Data Validation Module', '0.4', TO_DATE('', 'DD-MON-YY'), 'Updated the DVM_PKG procedures to added debugging comments using the DB Logging Module, this version of the DVM requires version 0.1 of the Database Logging Module Database (Git URL: git@gitlab.pifsc.gov:centralized-data-tools/centralized-tools.git in the DB_log folder)');
