    ...
    ELSE
        ...
        ELSE
            --the user was not authenticated via ICAM, this is not a valid application user:
						RAISE_APPLICATION_ERROR (-20000, 'The user credentials could not be verified');
...
--            RETURN FALSE;
    ...
    EXCEPTION
        ...
        WHEN NO_DATA_FOUND THEN
						RAISE_APPLICATION_ERROR (-20001, 'The user is not authorized to access the application');

--            RETURN FALSE;

        --Check if the active user query returned more than one row
        WHEN TOO_MANY_ROWS THEN
						RAISE_APPLICATION_ERROR (-20002, 'The user account is invalid');
--            RETURN FALSE;
...
INSERT INTO DB_UPGRADE_LOGS (UPGRADE_APP_NAME, UPGRADE_VERSION, UPGRADE_DATE, UPGRADE_DESC) VALUES ('Authorization Application Database', '1.0', TO_DATE('17-FEB-22', 'DD-MON-YY'), 'Updated AUTH_APP_PKG.APX_CUST_AUTH_USER package function to raise exceptions with error messages instead of returning false');
